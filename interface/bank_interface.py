...
def recharge_interface(username, amount):
    ...
    # 3.记录流水
    msg = f'\n{datetime.now()} 用户: {username} 充值{amount}元，成功！' \
...
def transfer_interface(username, to_username, amount):
    ...
    # 4.记录流水
    msg = f'\n{datetime.now()} 用户：{username} 给用户：{to_username} 转账：{amount}元 成功' \
          f'\n当前余额为：{user_data.get("balance")}元'
    user_data['flow'].append(msg)

    to_msg = f'\n{datetime.now()} 用户：{to_username} 收到用户：{username} 转账：{amount}元 成功' \
             f'\n当前余额为：{to_user_data.get("balance")}元'
    to_user_data['flow'].append(msg)
